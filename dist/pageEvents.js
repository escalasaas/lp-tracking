...
function mirrorClarityEvent(name) {
    try {
...
function mirrorClarityTags(experimentId, variant) {
    try {
 ...
 */
function sendEvents(events, experiment) {
    if (events.length === 0) {
        return;
    }
    ...
    for (const ctx of contexts) {
        ...
        try {
            if (typeof navigator !== "undefined" && typeof navigator.sendBeacon === "function") {
                sent = navigator.sendBeacon(getTrackingConfig().eventsPath, new Blob([body], { type: "application/json" }));
            }
        }
        catch {
            sent = false;
...
export function trackPageEvent(name, properties, experiment) {
    sendEvents([{ name, ...(properties ? { properties } : {}) }], experiment);
}
/**
